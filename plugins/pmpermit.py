...
from Extre import ALIVE_NAME, CMD_HELP, Andencento
from Extre.utils import extremepro_cmd


CUSTOM_PMPERMIT = os.environ.get("CUSTOM_PMPERMIT", None) or "This is Pm Security of Andencento"
...
@Andencento.on(extremepro_cmd(pattern="a ?(.*)"))
@Andencento.on(extremepro_cmd(pattern="approve ?(.*)"))
...
@Andencento.on(extremepro_cmd(pattern="da ?(.*)"))
@Andencento.on(extremepro_cmd(pattern="disapprove ?(.*)"))
async def approve_p_m(event):
    if event.fwd_from:
        return
    replied_user = await event.client(GetFullUserRequest(event.chat_id))
...
async def approve_p_m(event):
    ...
    APPROVED_PMs = "[Andencento] Currently Approved PMs\n"
    ...
    if len(APPROVED_PMs) > 4095:
        with io.BytesIO(str.encode(APPROVED_PMs)) as out_file:
            ...
            await event.client.send_file(
                ...
                force_document=True,
                allow_cache=False,
                caption="[Andencento]Current Approved PMs",
                reply_to=event,
            )
            await event.delete()
    else:
...
async def on_new_private_message(event):
    ...
    if USER_BOT_NO_WARN == message_text:
        ...
        return
    chat_ids = event.chat_id or message.chat_id == await message.get_chat()
...
CMD_HELP.update(
    {
        ...
        \nGet help from @AndencentoSupport."
    }
)
