...
@Andencento.on(admin_cmd(pattern=r"magnet"))
...
@Andencento.on(admin_cmd(pattern=r"tor"))
...
@Andencento.on(admin_cmd(pattern=r"url"))
async def magnet_download(event):
	if event.fwd_from:
		return
	var = event.text[5:]
...
@Andencento.on(admin_cmd(pattern=r"ariaRM"))
...
@Andencento.on(admin_cmd(pattern=r"show"))
