...
@client.on(admin_cmd(pattern=r"magnet"))
...
@client.on(admin_cmd(pattern=r"tor"))
...
@client.on(admin_cmd(pattern=r"url"))
async def magnet_download(event):
	if event.fwd_from:
		return
	var = event.text[5:]
...
@client.on(admin_cmd(pattern=r"ariaRM"))
...
@client.on(admin_cmd(pattern=r"show"))
