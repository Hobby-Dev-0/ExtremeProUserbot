...
@client.on(events.NewMessage(pattern=r"\.lslocal", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
...
@client.on(events.NewMessage(pattern=r"\.lsroot", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    DELAY_BETWEEN_EDITS = 0.3
...
@client.on(events.NewMessage(pattern=r"\.lssaved", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    DELAY_BETWEEN_EDITS = 0.3
    ...
    if stderr.decode():
        ...
        return
    await event.edit(f"{OUTPUT}`{stdout.decode()}`")
@client.on(events.NewMessage(pattern=r"\.rnsaved ?(.*)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    DELAY_BETWEEN_EDITS = 0.3
...
@client.on(events.NewMessage(pattern=r"\.rnlocal ?(.*)", outgoing=True))
async def _(event):
    if event.fwd_from:
        return
    DELAY_BETWEEN_EDITS = 0.3
...
@client.on(events.NewMessage(pattern=r"\.delsave (.*)", outgoing=True))
...
@client.on(events.NewMessage(pattern=r"\.delocal (.*)", outgoing=True))
async def handler(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
