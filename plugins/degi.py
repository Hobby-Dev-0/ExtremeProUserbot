...
@client.on(admin_cmd(pattern="degi$"))
@client.on(sudo_cmd(pattern="degi$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@client.on(admin_cmd(pattern=f"nehi$", outgoing=True))
@client.on(sudo_cmd(pattern=f"nehi$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
