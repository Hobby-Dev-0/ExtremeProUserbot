...
@client.on(admin_cmd(pattern="ud (.*)"))
@client.on(sudo_cmd(pattern="ud (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@client.on(admin_cmd(pattern="meaning (.*)"))
@client.on(sudo_cmd(pattern="meaning (.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    word = event.pattern_match.group(1)
