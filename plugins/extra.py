...
@Andencento.on(admin_cmd("ccry$"))
#@register(outgoing=True, pattern="^.cry$")
async def cry(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("(;´༎ຶД༎ຶ)")

@Andencento.on(admin_cmd("fp$"))
#@register(outgoing=True, pattern="^.fp$")
async def facepalm(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("🤦‍♂")

@Andencento.on(admin_cmd("moon$"))
...
@Andencento.on(admin_cmd("source$"))
#@register(outgoing=True, pattern="^.source$")
async def source(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit("/HellBoy-OP/LEGENDBOT")

@Andencento.on(admin_cmd("readme$"))
...
@Andencento.on(admin_cmd(pattern="evil ?(.*)"))
async def _(event):
