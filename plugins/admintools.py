...
async def demote(dmod):
    ...
    # Assume we don't have permission to demote
        ...
        return
    await dmod.edit(f" Success Demoted By EXTREMEPRO USERBOT ")
...
async def ban(bon):
    ...
    if reason:
        await bon.edit(f"𝙱𝚊𝚗𝚗𝚎𝚍 `{str(user.id)}` This Guy BY EXTREMEPRO BOT  !!\n\nReason: {reason}")
    else:
        await bon.edit(f"BANNED `{str(user.id)}` This Guy BY EXTREMEPRO BOT !!")
...
async def spider(spdr):
    ...
    try:
        from sql.modules.sql_helper.spam_mute_sql import mute
...
async def unmoot(unmot):
    ...
    # Check if the function running under SQL mode
    try:
        from SQL.modules.sql_helper.spam_mute_sql import unmute
    except AttributeError:
...
async def muter(moot):
    ...
    try:
        from DATABASE.modules.sql_helper.spam_mute_sql import is_muted
        from DATABASE.modules.sql_helper.gmute_sql import is_gmuted
    except AttributeError:
        return
...
async def ungmoot(un_gmute):
    ...
    # Check if the function running under SQL mode
    try:
        from DATABASE.modules.sql_helper.gmute_sql import ungmute
    except AttributeError:
...
async def gspider(gspdr):
    ...
    # Check if the function running under SQL mode
    try:
        from DATABASE.modules.sql_helper.gmute_sql import gmute
    except AttributeError:
